...
import { trustedAppOrigin } from "@/lib/auth-redirect";
...
async function sessionToken() { return (await cookies()).get("faturapp_session")?.value ?? null; }
const UUID_PATTERN = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
const GENERIC_RESOURCE_ERROR = "Lançamento não encontrado.";
function validatePassword(password: string) { return password.length >= 6 && /[A-Z]/.test(password) && /[0-9]/.test(password) && /[^A-Za-z0-9]/.test(password); }
export async function loginUser(login: string, password: string) { if (!login.trim() || !password) return { success: false, error: "Informe login e senha." }; const supabase = createClientServer(); const { data, error } = await supabase.rpc("app_login", { p_login: login, p_password: password }); if (error) { console.error("Erro Supabase login:", error); return { success: false, error: "Erro ao validar acesso. Tente novamente." }; } if (!data?.[0]) return { success: false, error: "Login ou senha incorretos." }; await setSessionCookie(data[0].session_token); return { success: true }; }
export async function registerUser(login: string, password: string, email: string) { const normalized = login.trim(); const normalizedEmail = email.trim().toLowerCase(); if (!normalized) return { success: false, error: "Informe um login." }; if (normalized.length > 120) return { success: false, error: "O login deve ter no máximo 120 caracteres." }; if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalizedEmail)) return { success: false, error: "Informe um e-mail válido para recuperar sua conta." }; if (!validatePassword(password)) return { success: false, error: "A senha deve ter no mínimo 6 caracteres, uma letra maiúscula, um número e um caractere especial." }; const supabase = createClientServer(); const { error: authError } = await supabase.auth.signUp({ email: normalizedEmail, password }); if (authError && !/already registered|already exists/i.test(authError.message)) { console.error("Erro Supabase cadastro Auth:", authError.code); if (/at least 6 characters|weak password/i.test(authError.message)) return { success: false, error: "A senha deve ter no mínimo 6 caracteres, uma letra maiúscula, um número e um caractere especial." }; return { success: false, error: "Não foi possível preparar a recuperação por e-mail. Tente novamente." }; } const { data, error } = await supabase.rpc("app_register", { p_login: normalized, p_password: password }); if (error || !data?.[0]) return { success: false, error: error?.message || "Não foi possível cadastrar." }; const { error: emailError } = await supabase.rpc("app_set_recovery_email", { p_token: data[0].session_token, p_email: normalizedEmail }); if (emailError) return { success: false, error: "A conta foi criada, mas não foi possível salvar o e-mail de recuperação." }; await setSessionCookie(data[0].session_token); revalidatePath("/"); revalidatePath("/relatorios"); return { success: true, role: data[0].role }; }
export async function requestPasswordRecovery(email: string) { const normalizedEmail = email.trim().toLowerCase(); if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalizedEmail)) return { success: false, error: "Informe um e-mail válido." }; const callback = `${trustedAppOrigin()}/auth/callback?next=${encodeURIComponent("/redefinir-senha")}`; const { error } = await createClientServer().auth.resetPasswordForEmail(normalizedEmail, { redirectTo: callback }); if (error) console.error("Erro ao solicitar recuperação:", error); return { success: true }; }
export async function finishPasswordRecovery(sessionToken: string) {
 if (!UUID_PATTERN.test(sessionToken)) return { success: false, error: "Sessão de recuperação inválida." };
 const { data, error } = await createClientServer().rpc("app_get_session", { p_token: sessionToken });
 if (error || !data?.[0]) return { success: false, error: "Sessão de recuperação inválida." };
 await setSessionCookie(sessionToken);
 revalidatePath("/"); revalidatePath("/relatorios");
 return { success: true };
}

export async function logoutUser() { const token = await sessionToken(); if (token) await createClientServer().rpc("app_logout", { p_token: token }); await clearSessionCookie(); redirect("/login"); }
