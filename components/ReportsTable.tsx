...
import { computeFeeAmount, computeFuelCost, computeFuelLiters, computeKmPerLiter, computeNetFare, formatBRL, formatDateBR, toNumber, type DailyEntry } from "@/lib/utils";
...
function maintenanceDetails(entry: DailyEntry): string[] {
  return (entry.maintenance_details || []).filter((item) => toNumber(item.value) !== 0 || item.description).map((item) => `${formatBRL(toNumber(item.value))} ${item.description || "Item sem descrição"}`);
}
function extrasDetails(entry: DailyEntry): string[] {
  return (entry.extra_expenses || []).filter((item) => toNumber(item.value) !== 0 || item.name).map((item) => `${formatBRL(toNumber(item.value))} ${item.name || "Item sem descrição"}`);
}
function DetailCell({ items, total }: { items: string[]; total: number }) {
  return <div className="text-right leading-5 whitespace-normal min-w-[150px]">{items.length > 0 && <div className="space-y-0.5 mb-1">{items.map((item, i) => <div key={i}>{item}</div>)}</div>}<div className="font-semibold border-t border-slate-200 pt-1">Total: {formatBRL(total)}</div></div>;
}
...
export default function ReportsTable({ entries, initialFrom, initialTo }: Props) {
  ...
  const calculateValues = (e: DailyEntry) => {
    ...
    const fuelCost = Math.max(0, Number(e.gas_expense) || 0) + Math.max(0, Number(e.alcohol_expense) || 0);
    const kmPerLiter = liters > 0 ? km / liters : null;
    return { gross, feePercent, feeAmount, net, gas, alcohol, maintenance, extras, profit, km, liters, fuelCost, kmPerLiter };
  };
  ...
  const totals = useMemo(() => filtered.reduce((acc, e) => {
    ...
    acc.km += v.km; acc.liters += v.liters; acc.fuelCost += v.fuelCost;
    return acc;
  }, { gross: 0, feeAmount: 0, net: 0, gas: 0, alcohol: 0, maintenance: 0, extras: 0, profit: 0, km: 0, liters: 0, fuelCost: 0 }), [filtered, cats]);

  const totalKmPerLiter = totals.liters > 0 ? totals.km / totals.liters : null;

  const summaryText = useMemo(() => {
    const lines = [`Relatório de ${formatDateBR(from)} a ${formatDateBR(to)}`, `Quilometragem total: ${formatKm(totals.km)} km`, `Combustível gasto: ${formatBRL(totals.fuelCost)}`, `Combustível abastecido: ${formatLiters(totals.liters)} L`, `Consumo: ${formatConsumption(totalKmPerLiter)}`, `Receita bruta: ${formatBRL(totals.gross)}`, `Taxas descontadas: ${formatBRL(totals.feeAmount)}`, `Receita líquida: ${formatBRL(totals.net)}`];
    ...
    return lines.join("\n");
  }, [from, to, totals, cats, totalKmPerLiter]);
  ...
  function downloadPDF() {
    ...
    doc.text(`Km rodados: ${formatKm(totals.km)} km`, 14, 27);
    doc.text(`Combustível: ${formatBRL(totals.fuelCost)}`, 75, 27);
    doc.text(`Litros: ${formatLiters(totals.liters)} L`, 140, 27);
    doc.text(`Consumo: ${formatConsumption(totalKmPerLiter)}`, 200, 27);

    const head = ["Data", "Km", "Litros", "Km/L", "Combustível", "Bruto", "Taxa", "Desconto da taxa", "Receita líquida"];
    if (cats.gas) head.push("Gasolina"); if (cats.alcohol) head.push("Álcool"); if (cats.maintenance) head.push("Manutenção"); if (cats.extras) head.push("Extras"); head.push("Lucro do Dia");
    const body = filtered.map((e) => {
      const v = calculateValues(e);
      const row: string[] = [formatDateBR(e.date), `${formatKm(v.km)}`, `${formatLiters(v.liters)} L`, formatConsumption(v.kmPerLiter), formatBRL(v.fuelCost), v.gross > 0 ? formatBRL(v.gross) : "—", v.gross > 0 ? `${v.feePercent.toFixed(2)}%` : "—", v.gross > 0 ? `− ${formatBRL(v.feeAmount)}` : "—", formatBRL(v.net)];
    ...
    const totalRow: string[] = ["TOTAL", formatKm(totals.km), `${formatLiters(totals.liters)} L`, formatConsumption(totalKmPerLiter), formatBRL(totals.fuelCost), formatBRL(totals.gross), "", `− ${formatBRL(totals.feeAmount)}`, formatBRL(totals.net)];
    if (cats.gas) totalRow.push(formatBRL(totals.gas)); if (cats.alcohol) totalRow.push(formatBRL(totals.alcohol)); if (cats.maintenance) totalRow.push(`TOTAL: ${formatBRL(totals.maintenance)}`); if (cats.extras) totalRow.push(`TOTAL: ${formatBRL(totals.extras)}`); totalRow.push(formatBRL(totals.profit)); body.push(totalRow);
    autoTable(doc, { head: [head], body, startY: 33, theme: "grid", styles: { fontSize: 5.7, cellPadding: 1.5, valign: "middle" }, headStyles: { fontSize: 5.7, fontStyle: "bold" }, columnStyles: { 0: { cellWidth: 18 } }, didParseCell: (data) => { if (data.row.index === body.length - 1) data.cell.styles.fontStyle = "bold"; }, margin: { left: 6, right: 6 } });
    doc.save(`FaturApp_Relatorio_${from}_${to}.pdf`);
  }

  const columnCount = 10 + Number(cats.gas) + Number(cats.alcohol) + Number(cats.maintenance) + Number(cats.extras);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm"><p className="text-xs text-slate-500">Km rodados</p><p className="text-2xl font-bold text-slate-800">{formatKm(totals.km)} km</p><p className="text-xs text-slate-500 mt-1">No período selecionado</p></div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm"><p className="text-xs text-slate-500">Combustível</p><p className="text-2xl font-bold text-slate-800">{formatBRL(totals.fuelCost)}</p><p className="text-xs text-slate-500 mt-1">Gasolina + álcool</p></div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm"><p className="text-xs text-slate-500">Litros</p><p className="text-2xl font-bold text-slate-800">{formatLiters(totals.liters)} L</p><p className="text-xs text-slate-500 mt-1">Total abastecido</p></div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm"><p className="text-xs text-slate-500">Consumo médio</p><p className="text-2xl font-bold text-slate-800">{formatConsumption(totalKmPerLiter)}</p><p className="text-xs text-slate-500 mt-1">Km total ÷ litros totais</p></div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-x-auto"><table className="min-w-full text-sm border-collapse border border-slate-300"><thead className="bg-slate-100 text-slate-700"><tr>
        ...
        {filtered.length === 0 ? <tr><td colSpan={columnCount} className="border border-slate-200 px-3 py-6 text-center text-slate-500">Nenhum lançamento no período.</td></tr> : filtered.map((e) => { const v = calculateValues(e); return <tr key={e.id} className="hover:bg-slate-50">
          <td className="border border-slate-200 px-3 py-2">{formatDateBR(e.date)}</td><td className="border border-slate-200 px-3 py-2 text-right">{formatKm(v.km)}</td><td className="border border-slate-200 px-3 py-2 text-right">{formatLiters(v.liters)} L</td><td className="border border-slate-200 px-3 py-2 text-right font-medium">{formatConsumption(v.kmPerLiter)}</td><td className="border border-slate-200 px-3 py-2 text-right">{formatBRL(v.fuelCost)}</td><td className="border border-slate-200 px-3 py-2 text-right">{v.gross > 0 ? formatBRL(v.gross) : "—"}</td><td className="border border-slate-200 px-3 py-2 text-right">{v.gross > 0 ? `${v.feePercent.toFixed(2)}%` : "—"}</td><td className="border border-slate-200 px-3 py-2 text-right text-red-600">{v.gross > 0 ? `− ${formatBRL(v.feeAmount)}` : "—"}</td><td className="border border-slate-200 px-3 py-2 text-right font-medium">{formatBRL(v.net)}</td>
          {cats.gas && <td className="border border-slate-200 px-3 py-2 text-right">{formatBRL(v.gas)}</td>}{cats.alcohol && <td className="border border-slate-200 px-3 py-2 text-right">{formatBRL(v.alcohol)}</td>}{cats.maintenance && <td className="border border-slate-200 px-3 py-2 align-top"><DetailCell items={maintenanceDetails(e)} total={v.maintenance} /></td>}{cats.extras && <td className="border border-slate-200 px-3 py-2 align-top"><DetailCell items={extrasDetails(e)} total={v.extras} /></td>}<td className={`border border-slate-200 px-3 py-2 text-right font-semibold ${v.profit >= 0 ? "text-brand-700" : "text-red-600"}`}>{formatBRL(v.profit)}</td>
        </tr>; })}
      </tbody><tfoot className="bg-slate-100 font-semibold"><tr>
        <td className="border border-slate-200 px-3 py-2">TOTAIS</td><td className="border border-slate-200 px-3 py-2 text-right">{formatKm(totals.km)}</td><td className="border border-slate-200 px-3 py-2 text-right">{formatLiters(totals.liters)} L</td><td className="border border-slate-200 px-3 py-2 text-right">{formatConsumption(totalKmPerLiter)}</td><td className="border border-slate-200 px-3 py-2 text-right">{formatBRL(totals.fuelCost)}</td><td className="border border-slate-200 px-3 py-2 text-right">{formatBRL(totals.gross)}</td><td className="border border-slate-200 px-3 py-2 text-right">—</td><td className="border border-slate-200 px-3 py-2 text-right text-red-600">− {formatBRL(totals.feeAmount)}</td><td className="border border-slate-200 px-3 py-2 text-right">{formatBRL(totals.net)}</td>
