...
function formatInputNumber(value:number){return value>0?String(value).replace(".",","):"";}
...
function draftDistance(draft:DraftState){return calculateDistance(draft.kmInitial,draft.kmFinal);}
...
function draftMetrics(draft:DraftState){
 ...
 const gasLiters=calculateFuelLiters(draft.gas,draft.gasPrice)+gasPurchases.reduce((total,item)=>total+calculateFuelLiters(item.amount,item.pricePerLiter),0),alcoholLiters=calculateFuelLiters(draft.alcohol,draft.alcoholPrice)+alcoholPurchases.reduce((total,item)=>total+calculateFuelLiters(item.amount,item.pricePerLiter),0);
 const km=draftDistance(draft),fuelMetrics=calculateFuelMetrics({distanceKm:km,gasCost:gas,alcoholCost:alcohol,gasLiters,alcoholLiters,referenceConsumptionKmPerLiter:draft.fuelConsumption}),totalLiters=fuelMetrics.totalLiters,totalFuel=fuelMetrics.totalFuelCost,weightedPrice=fuelMetrics.weightedPricePerLiter,consumption=fuelMetrics.kmPerLiter,consumptionMode=fuelMetrics.source,consumed=fuelMetrics.consumedLiters,consumedCost=fuelMetrics.consumedCost,maintenance=draft.maintenanceItems.filter(item=>item.description.trim()!==""),extras=draft.extras.filter(item=>item.name.trim()!==""),maintenanceTotal=maintenance.reduce((total,item)=>total+toNumber(item.value),0),extrasTotal=extras.reduce((total,item)=>total+toNumber(item.value),0),expensesTotal=revenue.taxaValor+totalFuel+maintenanceTotal+extrasTotal,profit=revenue.bruto-expensesTotal,totalCost=expensesTotal,costPerKm=km>0?totalCost/km:null,grossPerKm=km>0?revenue.bruto/km:null,profitPerKm=km>0?profit/km:null,hours=draft.hoursSegments.reduce((total,segment)=>{const [sh,sm]=segment.start.split(":").map(Number),[eh,em]=segment.end.split(":").map(Number);let minutes=(eh*60+em)-(sh*60+sm);if(minutes<0)minutes+=1440;return total+minutes/60;},0);
...
function aggregateDraftMetrics(date:string,drafts:DraftState[]):DraftMetric{
 ...
 const draftFuelMetrics=drafts.map(draft=>draftMetrics(draft));
 const totalLiters=draftFuelMetrics.reduce((total,item)=>total+item.totalLiters,0);
 const totalGasLiters=draftFuelMetrics.reduce((total,item)=>total+item.gasLiters,0),totalAlcoholLiters=draftFuelMetrics.reduce((total,item)=>total+item.alcoholLiters,0);
 const referenceConsumption=[...drafts].reverse().find(draft=>draft.fuelConsumption>0)?.fuelConsumption??0;
 const fuelMetrics=calculateFuelMetrics({distanceKm:km,gasCost:metrics.gas,alcoholCost:metrics.alcohol,gasLiters:totalGasLiters,alcoholLiters:totalAlcoholLiters,referenceConsumptionKmPerLiter:referenceConsumption});
 return {...metrics,gasLiters:totalGasLiters,alcoholLiters:totalAlcoholLiters,gasPrice:totalGasLiters>0?metrics.gas/totalGasLiters:0,alcoholPrice:totalAlcoholLiters>0?metrics.alcohol/totalAlcoholLiters:0,totalLiters,weightedPrice:fuelMetrics.weightedPricePerLiter,km,kmFinal:metrics.kmInitial+km,consumption:fuelMetrics.kmPerLiter,consumptionMode:fuelMetrics.source,consumed:fuelMetrics.consumedLiters,consumedCost:fuelMetrics.consumedCost,costPerKm:km>0?metrics.totalCost/km:null,grossPerKm:km>0?metrics.revenue.bruto/km:null,profitPerKm:km>0?metrics.profit/km:null};
}
...
export default function EntryForm({initialDate=hojeBrasilia(),initialMonthProfit=0,userId}:Props){
 ...
 function resetForm(){
  ...
  setKmInitial(0); setKmFinal(0); setFuelConsumption(0); setFuelConsumptionInput("");
 ...
 function updateVehicleConsumption(raw:string){
  setFuelConsumptionInput(raw);
  setFuelConsumption(toNumber(raw));
 }
 ...
 const currentMetrics=draftMetrics(currentDraft());
 ...
 const hasCurrentLaunch=fareNet!==0||totalFuelPurchase!==0||maintenanceTotal!==0||extrasSum!==0||kmDriven>0||fuelConsumption>0||hours>0;
 const percentageBaseValue=revenueSummary.bruto+netFare+additionalNetRevenue;
 const consolidatedSavedMetrics=useMemo(()=>aggregateDraftMetrics(date,savedDrafts),[date,savedDrafts]);
 const consolidatedSavedProfit=profitFromMetrics(consolidatedSavedMetrics);
 const dailyDrafts=useMemo(()=>hasCurrentLaunch?[...savedDrafts,currentDraft()]:savedDrafts,[hasCurrentLaunch,savedDrafts,date,mode,netFare,netApp,netCustomApp,revenueItems,netRevenueItems,gas,alcohol,gasPrice,alcoholPrice,fuelPurchases,kmInitial,kmFinal,fuelConsumption,hoursSegments,maintenanceItems,extras]);
 const dailyMetrics=useMemo(()=>aggregateDraftMetrics(date,dailyDrafts),[date,dailyDrafts]);
 ...
 const shouldAskVehicleConsumption=(kmInitial>0||kmFinal>0)&&fuelConsumption<=0;
 ...
 async function handleSubmit(e:FormEvent){
  ...
  const consolidated=aggregateDraftMetrics(date,recordsToClose);
  ...
  if(draftHasData(draftBeingSubmitted)&&consolidated.km>0&&consolidated.consumption<=0){setStatus("ℹ️ Informe o valor abastecido e o preço por litro para calcular o consumo automaticamente, ou use a referência do veículo.");return;}
  setStatus("Salvando...");
 ...
 return <div className="flex flex-col gap-4"><form onSubmit={handleSubmit} className="order-2 space-y-3">
  ...
  <details className="group rounded-xl border border-slate-200 bg-white shadow-sm"><summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3"><span className="text-sm font-bold text-slate-800">🚗 Quilometragem</span><span className="flex items-center"><strong className="text-sm text-slate-700">{formatKm(kmDriven)} km</strong><DisclosureChevron/></span></summary><div className="space-y-3 border-t border-slate-100 p-3"><div className="grid grid-cols-2 gap-3"><div><label className="label">Km inicial</label><input type="number" step="0.1" min="0" className="input" value={kmInitial||""} onChange={e=>setKmInitial(toNumber(e.target.value))} placeholder="Ex.: 52.340"/></div><div><label className="label">Km final</label><input type="number" step="0.1" min="0" className="input" value={kmFinal||""} onChange={e=>setKmFinal(toNumber(e.target.value))} placeholder="Ex.: 52.520"/></div></div>
   <div><label className="label">Referência do veículo (km/L)</label><input type="text" inputMode="decimal" pattern="[0-9]*([,.][0-9]+)?" className="input" value={fuelConsumptionInput} onChange={e=>updateVehicleConsumption(e.target.value)} placeholder="Ex.: 12,2" aria-describedby="vehicle-consumption-help"/><p id="vehicle-consumption-help" className="mt-1 text-xs font-semibold text-slate-600">( Quantos KM/L de combustível seu veículo faz? )</p>{shouldAskVehicleConsumption&&<div role="alert" aria-live="polite" className="mt-2 rounded-xl border border-amber-300 bg-amber-50 px-3 py-2.5 text-sm font-bold text-amber-900 shadow-sm">📣 Informe quantos KM/L de combustível seu veículo faz para calcular o consumo do dia.</div>}</div></div></details>
 ...
 <section className="order-4 rounded-xl border border-slate-200 bg-slate-50/80 p-3 shadow-sm" aria-label="Resumo do dia até agora">
  ...
  {hasSavedLaunches&&<div className="mt-3 grid grid-cols-2 gap-2">
   <div className="rounded-lg border border-slate-200 bg-white px-2.5 py-2"><p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">💰 Lucro líquido acumulado</p><div className="mt-1 inline-flex items-center gap-1.5"><span aria-hidden="true" className={"h-1.5 w-1.5 rounded-full " + (consolidatedSavedProfit<0?"bg-red-600":"bg-emerald-600")}></span><strong className={"text-sm font-black " + (consolidatedSavedProfit<0?"text-red-600":"text-emerald-600")}>{formatBRL(consolidatedSavedProfit)}</strong></div></div>
   <div className="rounded-lg border border-slate-200 bg-white px-2.5 py-2"><p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">🚗 Estimativa km/L</p><strong className="mt-1 block text-sm font-black text-slate-800">{consolidatedSavedMetrics.consumption>0?`${formatKm(consolidatedSavedMetrics.consumption)} km/L`:"—"}</strong><p className="mt-0.5 text-[10px] text-slate-500">Não é medição tanque-a-tanque</p></div>
   <div className="rounded-lg border border-slate-200 bg-white px-2.5 py-2"><p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">💸 Custo total por KM</p><strong className="mt-1 block text-sm font-black text-slate-800">{formatCostPerKm(consolidatedSavedMetrics.costPerKm)}</strong></div>
   <div className="rounded-lg border border-slate-200 bg-white px-2.5 py-2"><p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">↗ Ganho por KM</p><strong className="mt-1 block text-sm font-black text-slate-800">{formatCostPerKm(consolidatedSavedMetrics.grossPerKm)}</strong></div>
   <div className="rounded-lg border border-slate-200 bg-white px-2.5 py-2"><p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">💹 Lucro por KM</p><strong className="mt-1 block text-sm font-black text-slate-800">{formatCostPerKm(consolidatedSavedMetrics.profitPerKm)}</strong></div>
  </div>}
 </section>
 <div className="sticky top-2 z-10 order-1">
  <section className={"rounded-xl border p-3 shadow-md backdrop-blur-sm " + (summaryTone==="profit"?"border-emerald-200 bg-emerald-50/95":summaryTone==="loss"?"border-red-200 bg-red-50/95":"border-amber-200 bg-amber-50/95")} aria-live="polite" aria-label="Resumo financeiro diário">
   ...
   <div className="mt-2 grid grid-cols-1 gap-2 border-t border-slate-200/80 pt-2 sm:grid-cols-4 sm:divide-x sm:divide-slate-200/80">
    ...
    <div className="sm:pl-2"><p className="text-sm font-semibold leading-snug text-slate-700">{dailyConsumptionMode==="automatic"?<>Estimativa do trecho: <strong className="font-black text-slate-900">{formatKm(dailyConsumption)} km/L</strong><span className="mt-0.5 block text-xs font-medium text-slate-500">Não é consumo real sem medição tanque-a-tanque.</span></>:dailyConsumptionMode==="reference"?<>Estimativa pela referência do veículo: <strong className="font-black text-slate-900">{formatKm(dailyConsumption)} km/L</strong></>:<>Informe km e combustível para calcular a <strong className="font-black text-slate-900">estimativa km/L</strong></>}</p></div>
