...
import { formatBRL, formatDateTimeBrasilia, hojeBrasilia, toNumber } from "@/lib/utils";
...
import { calculateDay, type DayCalculationContext, type DayLaunchInput, type DayLaunchResult } from "@/lib/day-calculation";
...
type StoredVehicleProfile = Required<DayCalculationContext> & { sourceDate:string; baseContext?:DayCalculationContext };
type FinancialEngineContext = DayCalculationContext;
type RemovedLaunch = { record:LaunchRecord; index:number };

function scopedStorageKey(userId:string, key:string){return `faturapp:user:${userId}:${key}`;}
function engineContextOnly(context:DayCalculationContext):DayCalculationContext{return {consumoPerfilKmL:context.consumoPerfilKmL,tipoCombustivel:context.tipoCombustivel,hodometroUltimoTanqueCheio:context.hodometroUltimoTanqueCheio,historicoConsumoKmL:context.historicoConsumoKmL,ultimoPrecoPorLitro:context.ultimoPrecoPorLitro,fuelRemainingLiters:context.fuelRemainingLiters,fuelRemainingValue:context.fuelRemainingValue};}
...
function validateDraftBeforeSave(draft:DraftState){
 if(draft.kmFinal<draft.kmInitial)return "O KM final não pode ser menor que o KM inicial.";
 if(draft.gas>0&&draft.gasPrice<=0)return "Informe o preço por litro da gasolina.";
 if(draft.alcohol>0&&draft.alcoholPrice<=0)return "Informe o preço por litro do álcool.";
 if((draft.fuelPurchases??[]).some(item=>item.amount>0&&item.pricePerLiter<=0))return "Informe o preço por litro em cada abastecimento.";
 if(draft.hoursSegments.some(item=>(item.start&&!item.end)||(!item.start&&item.end)))return "Informe o início e o fim de cada período trabalhado.";
 const grossItems=draft.revenueItems.filter(item=>toNumber(item.bruto)>0),netItems=(draft.netRevenueItems??[]).filter(item=>toNumber(item.bruto)>0),hasRevenue=draft.netFare>0||grossItems.length>0||netItems.length>0;
 if(hasRevenue&&draft.mode===null)return "Selecione se o valor é bruto com taxa ou já líquido.";
 if(draft.mode==="net"&&draft.netFare>0&&!draft.netApp)return "Selecione o aplicativo do valor líquido recebido.";
 if(draft.mode==="withFee"&&grossItems.some(item=>!item.app))return "Selecione o aplicativo de cada receita informada.";
 if(netItems.some(item=>!item.app))return "Selecione o aplicativo de cada ganho líquido informado.";
 if((draft.mode==="withFee"&&grossItems.length===0)||(draft.mode==="net"&&draft.netFare<=0&&netItems.length===0))return "Informe o valor da receita antes de salvar.";
 const hasRoadActivity=hasRevenue||normalizeTimeSegments(draft.hoursSegments).length>0||draft.fuelConsumption>0||Boolean(draft.fullTank);
 if(hasRoadActivity&&draftDistance(draft)<=0)return "Falta informar quantos km foram rodados para finalizar o cálculo";
 return null;
}

function draftHasData(draft:DraftState){return draft.netFare>0||draft.netCustomApp.trim()!==""||draft.revenueItems.some(item=>item.bruto>0||item.taxa>0||item.nomeAppPersonalizado.trim()!=="")||(draft.netRevenueItems??[]).some(item=>item.bruto>0)||draft.gas>0||draft.alcohol>0||draft.gasPrice>0||draft.alcoholPrice>0||(draft.fuelPurchases??[]).some(item=>item.amount>0||item.pricePerLiter>0)||Boolean(draft.fullTank)||draftDistance(draft)>0||draft.fuelConsumption>0||draft.hoursSegments.some(segment=>segment.start!==""||segment.end!=="")||draft.maintenanceItems.some(item=>toNumber(item.value)>0)||draft.extras.some(item=>toNumber(item.value)>0);}
...
function dayInput(draft:DraftState,meta:Partial<Pick<DayLaunchInput,"id"|"number"|"createdAt">>={}):DayLaunchInput{
 const facts=draftFacts(draft);
 const legacyPurchases:FuelPurchase[]=[
  ...(draft.gas>0||draft.gasPrice>0?[{id:`legacy-gas-${meta.id??draft.date}`,type:"gasoline" as const,amount:draft.gas,pricePerLiter:draft.gasPrice}]:[]),
  ...(draft.alcohol>0||draft.alcoholPrice>0?[{id:`legacy-alcohol-${meta.id??draft.date}`,type:"alcohol" as const,amount:draft.alcohol,pricePerLiter:draft.alcoholPrice}]:[]),
 ];
 return {...meta,date:draft.date,revenueItems:facts.revenueItems,hoursWorked:facts.hours,kmInitial:draft.kmInitial,kmFinal:draft.kmFinal,fuelPurchases:[...legacyPurchases,...(draft.fuelPurchases??[])],fuelType:facts.fuelType,fullTank:Boolean(draft.fullTank),consumptionKmL:draft.fuelConsumption,maintenanceItems:facts.maintenance,extraItems:facts.extras};
}
function metricFromResult(facts:ReturnType<typeof draftFacts>,result:DayLaunchResult){return {...facts,consumption:result.consumptionKmL,consumptionMode:result.consumptionMode,consumed:result.fuelConsumedLiters,consumedCost:result.fuelConsumedCost,isolatedFuelExpense:result.isolatedFuelExpense,expensesTotal:result.totalOutflows,totalCost:result.totalOutflows,operatingCosts:result.operatingCosts,profit:result.profit,costPerKm:result.km>0?result.costPerKm:null,grossPerKm:result.km>0?result.gainPerKm:null,profitPerKm:result.km>0?result.profitPerKm:null,fuelRemainingLiters:result.fuelRemainingLiters,fuelRemainingValue:result.fuelRemainingValue};}
type DraftMetric = ReturnType<typeof metricFromResult>;
function calculateDraftChain(date:string,drafts:DraftState[],context:DayCalculationContext={}){
 const facts=drafts.map(draftFacts),summary=calculateDay(drafts.map((draft,index)=>dayInput(draft,{id:`draft-${index}`})),context),launchMetrics=summary.launches.map((result,index)=>metricFromResult(facts[index],result));
 const mergedFacts=draftFacts(mergeDrafts(date,drafts));
 const metrics={...mergedFacts,revenue:{normalized:summary.revenueItems,bruto:summary.revenueGross,taxaValor:summary.fees,liquido:summary.revenueNet,taxaPercentual:summary.revenueGross>0?summary.fees/summary.revenueGross*100:0},revenueItems:summary.revenueItems,totalLiters:summary.fuelPurchasedLiters,totalFuel:summary.fuelPurchasedAmount,latestPrice:summary.fuelPriceApplied,km:summary.km,kmFinal:mergedFacts.kmInitial+summary.km,hours:summary.hours,maintenanceTotal:summary.maintenance,extrasTotal:summary.extras,consumption:summary.consumptionKmL,consumptionMode:summary.consumptionMode,consumed:summary.fuelConsumedLiters,consumedCost:summary.fuelConsumedCost,isolatedFuelExpense:summary.isolatedFuelExpense,expensesTotal:summary.totalOutflows,totalCost:summary.totalOutflows,operatingCosts:summary.operatingCosts,profit:summary.profit,costPerKm:summary.km>0?summary.costPerKm:null,grossPerKm:summary.km>0?summary.gainPerKm:null,profitPerKm:summary.km>0?summary.profitPerKm:null,fuelRemainingLiters:summary.fuelRemainingLiters,fuelRemainingValue:summary.fuelRemainingValue,summary} as DraftMetric & {summary:typeof summary};
 return {launchMetrics,context:summary.context,metrics};
}
function draftMetrics(draft:DraftState,context:DayCalculationContext={}):DraftMetric{return calculateDraftChain(draft.date,[draft],context).metrics;}
function aggregateDraftMetrics(date:string,drafts:DraftState[],context:DayCalculationContext={}):DraftMetric & {summary:ReturnType<typeof calculateDay>}{return calculateDraftChain(date,drafts,context).metrics;}
...
function draftFinancialSummary(draft:DraftState,providedMetrics?:DraftMetric){
 ...
 return {profit:metrics.profit,fuelConsumedLiters:metrics.consumed,fuelConsumedCost:metrics.consumedCost,fuelPurchasedLiters:metrics.totalLiters,fuelPurchasedCost:metrics.totalFuel,fuelCostForProfit:metrics.consumedCost+metrics.isolatedFuelExpense,fuelRemainingLiters:metrics.fuelRemainingLiters,fuelRemainingValue:metrics.fuelRemainingValue};
}
function launchType(draft:DraftState){const facts=draftFacts(draft);if(facts.revenue.bruto>0)return "💰 Receita";if(facts.totalFuel>0)return "⛽ Abastecimento";if(facts.maintenanceTotal>0)return "🔧 Manutenção";if(facts.extrasTotal>0)return "🧾 Gasto extra";if(facts.hours>0)return "⏱️ Período trabalhado";return "🚗 Quilometragem";}
...
export default function EntryForm({initialDate=hojeBrasilia(),initialMonthProfit=0,userId}:Props){
 ...
 const [removedLaunch,setRemovedLaunch]=useState<RemovedLaunch|null>(null);
 const undoTimer=useRef<ReturnType<typeof setTimeout>|null>(null);

 function latestKmFinal(records=savedLaunches){return [...records].reverse().find(record=>record.draft.kmFinal>0)?.draft.kmFinal??0;}
 function resetForm(prefillKmInitial=latestKmFinal()){
  ...
  setKmInitial(prefillKmInitial); setKmFinal(prefillKmInitial); setFuelConsumption(0);
 ...
 function restoreDraft(selectedDate:string){
  ...
  const prefill=[...selectedLaunches].reverse().find(record=>record.draft.kmFinal>0)?.draft.kmFinal??0;
  setDate(selectedDate); setSavedLaunches(selectedLaunches); setDayClosed(closed); setEditingLaunchId(null); resetForm(prefill); setMode(draft.mode??null); setNetFare(draft.netFare||0); setNetApp(draft.netApp||""); setNetCustomApp(draft.netCustomApp||""); setRevenueItems(draft.revenueItems||[]); setNetRevenueItems(draft.netRevenueItems||[]); setGas(draft.gas||0); setAlcohol(draft.alcohol||0); setGasPrice(draft.gasPrice||0); setAlcoholPrice(draft.alcoholPrice||0); setFuelPurchases(draft.fuelPurchases||[]); setFuelType(draft.fuelType??"gasoline"); setFullTank(Boolean(draft.fullTank)); setKmInitial(draft.kmInitial??prefill); setKmFinal(draft.kmFinal??prefill); setFuelConsumption(draft.fuelConsumption||0); setHoursSegments(draft.hoursSegments?.length?draft.hoursSegments:[{start:"",end:""}]); setMaintenanceItems(draft.maintenanceItems||[]); setExtras(draft.extras||[]); setSavedCard(null);
 ...
 useEffect(()=>{
  ...
  const hasDraftData=netFare>0||netCustomApp.trim()!==""||revenueItems.some(item=>item.bruto>0||item.taxa>0||item.nomeAppPersonalizado.trim()!=="")||netRevenueItems.length>0||gas>0||alcohol>0||gasPrice>0||alcoholPrice>0||fuelPurchases.length>0||fullTank||kmFinal>kmInitial||fuelConsumption>0||hoursSegments.some(segment=>segment.start!==""||segment.end!=="")||maintenanceItems.length>0||extras.length>0;
 ...
 function saveDraft():boolean{
  ...
  try {
   ...
   const previous=editingLaunchId?savedLaunches.find(item=>item.id===editingLaunchId):undefined;
   const record:LaunchRecord={id:editingLaunchId??`launch-${Date.now()}-${Math.random().toString(36).slice(2,8)}`,number:editingLaunchId?(previous?.number??nextNumber):nextNumber,date,createdAt:previous?.createdAt??new Date().toISOString(),draft};
   ...
   resetForm(draft.kmFinal>0?draft.kmFinal:latestKmFinal(next));
 ...
 function removeLaunch(record:LaunchRecord){
  ...
  try {
   const index=savedLaunches.findIndex(item=>item.id===record.id),next=savedLaunches.filter(item=>item.id!==record.id);
   persistLaunches(next);persistVehicleProfile(next.map(item=>item.draft));
   if(editingLaunchId===record.id){setEditingLaunchId(null);resetForm(latestKmFinal(next));}
   if(undoTimer.current)clearTimeout(undoTimer.current);
   setRemovedLaunch({record,index:Math.max(0,index)});
   undoTimer.current=setTimeout(()=>{setRemovedLaunch(null);undoTimer.current=null;},6000);
 ...
 function undoRemove(){
  if(!removedLaunch)return;
  const next=[...savedLaunches];next.splice(Math.min(removedLaunch.index,next.length),0,removedLaunch.record);
  persistLaunches(next);persistVehicleProfile(next.map(item=>item.draft));
  if(undoTimer.current)clearTimeout(undoTimer.current);undoTimer.current=null;
  setRemovedLaunch(null);setStatus(`✅ Registro #${String(removedLaunch.record.number).padStart(3,"0")} restaurado.`);
 }
 ...
 function reopenDay(){
  ...
  try {
   ...
   setDayClosed(false);setEditingLaunchId(null);setSavedCard(null);setStatus("Dia reaberto. Você pode adicionar ou editar lançamentos.");
 ...
 const currentMetrics=draftMetrics(currentDraft(),currentContext);
 ...
 const inheritedConsumption=fuelConsumption<=0?nonNegative(currentContext.consumoPerfilKmL):0;
 const inheritedFuelPrice=currentMetrics.latestPrice<=0?nonNegative(currentContext.ultimoPrecoPorLitro):0;
 ...
 function selectRevenueMode(nextMode:Mode){setMode(nextMode);if(nextMode==="withFee"&&revenueItems.length===0)setRevenueItems([createRevenueItem()]);}
 ...
 async function handleSubmit(e:FormEvent){
  ...
  const draftBeingSubmitted=normalizeDraftForSave(rawDraft),hasDraftToClose=draftHasData(draftBeingSubmitted);
  const recordsToClose=savedLaunches.map(record=>({...record,draft:normalizeDraftForSave(record.draft)})).filter(record=>draftHasData(record.draft));
  let addedRecord:LaunchRecord|null=null;
  if(editingLaunchId){const index=recordsToClose.findIndex(record=>record.id===editingLaunchId);if(index>=0&&hasDraftToClose)recordsToClose[index]={...recordsToClose[index],draft:draftBeingSubmitted};}
  else if(hasDraftToClose){const nextNumber=Number(window.localStorage.getItem(nextLaunchNumberKey)||"1");addedRecord={id:`launch-${Date.now()}-${Math.random().toString(36).slice(2,8)}`,number:nextNumber,date,createdAt:new Date().toISOString(),draft:draftBeingSubmitted};recordsToClose.push(addedRecord);}
  if(recordsToClose.length===0){setStatus("❌ Nenhum lançamento preenchido para fechar o dia.");return;}
  const consolidated=aggregateDraftMetrics(date,recordsToClose.map(record=>record.draft),initialEngineContext);
  ...
  if(hasDraftToClose&&consolidated.km>0&&consolidated.consumption<=0){setStatus("ℹ️ Informe o consumo médio ou informe o valor abastecido e o preço por litro para calcular o consumo automaticamente.");return;}
  setStatus("Salvando...");
  let reopenHistory:DayReopenRecord[]=[];try{const raw=window.localStorage.getItem(reopenHistoryKey(date));if(raw){const parsed=JSON.parse(raw);if(Array.isArray(parsed))reopenHistory=parsed;}}catch{/* histórico inválido não impede o fechamento */}
  const launchDetails=recordsToClose.map(record=>dayInput(record.draft,{id:record.id,number:record.number,createdAt:record.createdAt}));
  const payload={date,gross_amount:consolidated.revenue.bruto,fee_percent:consolidated.revenue.taxaPercentual,net_fare:consolidated.revenue.liquido,revenue_details:consolidated.revenueItems,gas_expense:consolidated.gas,alcohol_expense:consolidated.alcohol,gasoline_price_per_liter:consolidated.gasPrice,alcohol_price_per_liter:consolidated.alcoholPrice,gasoline_liters:consolidated.gasLiters,alcohol_liters:consolidated.alcoholLiters,fuel_price_per_liter_current:consolidated.latestPrice,km_initial:consolidated.kmInitial,km_final:consolidated.kmFinal,km_driven:consolidated.km,hours_worked:consolidated.hours,fuel_consumption_km_per_liter:consolidated.consumption,fuel_consumed_liters:consolidated.consumed,fuel_consumed_cost:consolidated.consumedCost,isolated_fuel_expense:consolidated.isolatedFuelExpense,fuel_remaining_liters:consolidated.fuelRemainingLiters,fuel_remaining_value:consolidated.fuelRemainingValue,launch_details:launchDetails,reopen_history:reopenHistory,maintenance_expense:consolidated.maintenanceTotal,maintenance_details:consolidated.maintenance,extra_expenses:consolidated.extras};
  const res=await saveEntry(payload);
  if(res.success){if(addedRecord)window.localStorage.setItem(nextLaunchNumberKey,String(addedRecord.number+1));persistLaunches(recordsToClose);window.localStorage.removeItem(openDayKey(date));window.localStorage.setItem(closedDayKey(date),"1");persistVehicleProfile(recordsToClose.map(record=>record.draft));setDayClosed(true);setEditingLaunchId(null);setStatus("✅ Dia registrado e fechado com sucesso!");if(typeof res.monthProfit==="number")setMonthProfit(res.monthProfit);const consolidatedProfit=profitFromMetrics(consolidated);setSavedCard({profit:consolidatedProfit,km:consolidated.km,hours:consolidated.hours,profitPerKm:consolidated.profitPerKm,revenuePerKm:consolidated.grossPerKm,costPerKm:consolidated.costPerKm,revenueBase:consolidated.revenue.bruto});resetForm(latestKmFinal(recordsToClose));}else setStatus(`❌ Erro: ${res.error}`);
 }
 ...
 return <div className="flex flex-col gap-4"><form onSubmit={handleSubmit} className="order-2 space-y-3">
  ...
  <details className="group rounded-xl border border-slate-200 bg-white shadow-sm"><summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3"><span className="text-sm font-bold text-slate-800">💰 Receitas <span className="ml-1 text-xs font-normal text-slate-500">{revenueItems.filter(item=>item.bruto>0).length+(netFare>0?1:0)+netRevenueItems.filter(item=>item.bruto>0).length} lanç.</span></span><span className="flex items-center"><strong className="text-sm text-emerald-700">{formatBRL(fareNet)}</strong><DisclosureChevron/></span></summary><div className="space-y-3 border-t border-slate-100 p-3">
   <div className="flex flex-wrap gap-4"><label className="flex items-center gap-2 text-sm cursor-pointer"><input type="radio" checked={mode==="withFee"} onChange={()=>selectRevenueMode("withFee")}/>Valor bruto + taxa</label><label className="flex items-center gap-2 text-sm cursor-pointer"><input type="radio" checked={mode==="net"} onChange={()=>selectRevenueMode("net")}/>Valor já líquido</label></div>
   ...
   {mode==="net"&&<div className="space-y-3"><div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><div><label className="label">Aplicativo</label><select className="input" value={netApp} onChange={e=>setNetApp(e.target.value as RevenueAppName|"")}><option value="">Selecione o aplicativo</option>{REVENUE_APPS.map(app=><option key={app} value={app}>{app}</option>)}</select>{netApp==="Outro"&&<input className="input mt-2" value={netCustomApp} onChange={e=>setNetCustomApp(e.target.value)} placeholder="Nome do aplicativo"/>}</div><div><label className="label">Valor líquido recebido (R$)</label><input type="number" step="0.01" min="0" className="input" value={netFare||""} onChange={e=>setNetFare(toNumber(e.target.value))} placeholder="Ex.: 250,00"/></div></div>
    {netRevenueItems.map((item,index)=><details key={item.id} open={index===netRevenueItems.length-1?true:undefined} className="rounded-lg border border-slate-200 bg-slate-50/70"><summary className="flex cursor-pointer list-none items-center justify-between gap-2 px-3 py-2 text-sm"><span className="truncate font-medium text-slate-700">{item.app==="Outro"?(item.nomeAppPersonalizado||"Outro aplicativo"):(item.app||"Aplicativo não selecionado")}</span><strong className="text-emerald-700">{formatBRL(item.bruto)}</strong></summary><div className="space-y-3 border-t border-slate-200 p-3"><div className="flex items-center justify-between gap-2"><span className="text-xs font-bold text-slate-600">Ganho líquido {index+1}</span><button type="button" onClick={()=>removeNetRevenue(item.id)} className="rounded-lg bg-rose-50 px-3 py-1 text-xs font-bold text-rose-600">Remover</button></div><div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><div><label className="label">Aplicativo</label><select className="input" value={item.app} onChange={e=>updateNetRevenue(item.id,{app:e.target.value as RevenueAppName|"",nomeAppPersonalizado:e.target.value==="Outro"?item.nomeAppPersonalizado:""})}><option value="">Selecione o aplicativo</option>{REVENUE_APPS.map(app=><option key={app} value={app}>{app}</option>)}</select>{item.app==="Outro"&&<input className="input mt-2" value={item.nomeAppPersonalizado} onChange={e=>updateNetRevenue(item.id,{nomeAppPersonalizado:e.target.value})} placeholder="Nome do aplicativo"/>}</div><div><label className="label">Valor líquido recebido (R$)</label><input type="number" step="0.01" min="0" className="input" value={item.bruto||""} onChange={e=>updateNetRevenue(item.id,{bruto:toNumber(e.target.value)})} placeholder="Ex.: 40,00"/></div></div></div></details>)}
  ...
  <details className="group rounded-xl border border-slate-200 bg-white shadow-sm"><summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3"><span className="text-sm font-bold text-slate-800">🚗 Quilometragem</span><span className="flex items-center"><strong className="text-sm text-slate-700">{formatKm(kmDriven)} km</strong><DisclosureChevron/></span></summary><div className="space-y-3 border-t border-slate-100 p-3"><div className="grid grid-cols-2 gap-3"><div><label className="label">Km inicial</label><input type="number" step="0.1" min="0" className="input" value={kmInitial||""} onChange={e=>setKmInitial(toNumber(e.target.value))} placeholder="Ex.: 52.340"/></div><div><label className="label">Km final</label><input type="number" step="0.1" min="0" className="input" value={kmFinal||""} onChange={e=>setKmFinal(toNumber(e.target.value))} placeholder="Ex.: 52.520"/></div></div>
   <div><label className="label">Média de consumo do veículo (km/L)</label><input type="number" inputMode="decimal" step="0.1" min="0" className="input" value={fuelConsumption||""} onChange={e=>updateVehicleConsumption(toNumber(e.target.value))} placeholder="Ex.: 10 km/L"/><p className="mt-1 text-xs text-slate-500">Se informada, esta média será usada como nova referência. Entre dois registros de tanque cheio, o cálculo exato terá prioridade.</p>{inheritedConsumption>0&&<p className="mt-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-800">Usando a média de {formatKm(inheritedConsumption)} km/L do seu último lançamento{inheritedFuelPrice>0?` e o preço de ${formatBRL(inheritedFuelPrice)}/L`:""}.</p>}</div></div></details>
 ...
 {editingLaunchId&&<div className="order-3 rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm font-bold text-amber-900 shadow-sm">Rascunho aberto para editar. Depois de editar, salve o lançamento.</div>}
 <section className="order-4 rounded-xl border border-slate-200 bg-white p-3 shadow-sm" aria-label="Lançamentos do dia aberto">
  ...
  {removedLaunch&&<div className="mt-3 flex items-center justify-between gap-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-900"><span>Registro removido. O resumo já foi recalculado.</span><button type="button" onClick={undoRemove} className="rounded-md bg-amber-900 px-3 py-1.5 font-bold text-white">Desfazer</button></div>}
  {savedLaunches.length===0?<p className="mt-3 rounded-lg bg-slate-50 px-3 py-3 text-sm text-slate-500">Nenhum lançamento salvo ainda.</p>:<div className="mt-3 space-y-2">{savedLaunches.slice().sort((a,b)=>a.number-b.number).map(record=>{const recordIndex=savedLaunches.findIndex(item=>item.id===record.id),summary=draftFinancialSummary(record.draft,displaySavedMetrics[recordIndex]),created=formatDateTimeBrasilia(record.createdAt),isEditing=editingLaunchId===record.id,fuelLiters=summary.fuelConsumedLiters>0?summary.fuelConsumedLiters:summary.fuelPurchasedLiters,fuelText=summary.fuelCostForProfit>0?`Combustível gasto ${formatLiters(fuelLiters)} L (${formatBRL(summary.fuelCostForProfit)})`:"Combustível gasto —",remainingText=`Restante no tanque ${formatLiters(summary.fuelRemainingLiters)} L (${formatBRL(summary.fuelRemainingValue)})`;return <div key={record.id} className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2"><div className="flex flex-wrap items-center justify-between gap-2"><div><strong className="text-sm text-slate-800">#{String(record.number).padStart(3,"0")}</strong><span className="ml-2 text-xs font-bold text-slate-600">{launchType(record.draft)}</span></div><div className="flex flex-wrap items-center justify-end gap-2"><span className="text-xs font-medium text-slate-500">{created.date} · {created.time}</span><button type="button" disabled={dayClosed} onClick={()=>isEditing?saveDraft():loadLaunchIntoForm(record)} className="rounded-md border border-emerald-200 bg-white px-2 py-1 text-[11px] font-bold text-emerald-700 transition hover:bg-emerald-50 disabled:cursor-not-allowed disabled:opacity-50">{isEditing?"salvar":"editar"}</button><button type="button" disabled={dayClosed} onClick={()=>removeLaunch(record)} className="rounded-md border border-rose-200 bg-white px-2 py-1 text-[11px] font-bold text-rose-600 transition hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-50">remover</button></div></div><p className="mt-1 text-xs text-slate-600">Lucro líquido <span className="inline-flex items-center gap-1 font-bold"><span aria-hidden="true" className={"h-1.5 w-1.5 rounded-full " + (summary.profit<0?"bg-red-600":"bg-emerald-600")}></span><span className={summary.profit<0?"text-red-600":"text-emerald-600"}>{formatBRL(summary.profit)}</span></span> · {fuelText} · {remainingText}</p></div>})}</div>}
 </section>
 <div className="sticky top-2 z-10 order-1">
  <section className={"rounded-xl border p-3 shadow-md backdrop-blur-sm " + (summaryTone==="profit"?"border-emerald-200 bg-emerald-50/95":summaryTone==="loss"?"border-red-200 bg-red-50/95":"border-amber-200 bg-amber-50/95")} aria-live="polite" aria-label="Resumo financeiro diário">
   ...
   <div className="mt-2 grid grid-cols-1 gap-2 border-t border-slate-200/80 pt-2 sm:grid-cols-4 sm:divide-x sm:divide-slate-200/80">
    ...
    <div className="sm:pl-2"><p className="text-sm font-semibold leading-snug text-slate-700">Seu carro faz a média de <strong className="font-black text-slate-900">{formatKm(dailyConsumption)} km</strong> por litro de combustível{dailyConsumptionMode==="exato_tanque_cheio"?" (cálculo entre tanques cheios)":dailyConsumptionMode==="estimativa_inicial"?" (estimativa inicial)":""}</p></div>
   ...
   <details className="group mt-2 border-t border-slate-200/80 pt-2"><summary className="flex cursor-pointer list-none items-center justify-center text-[10px] font-bold uppercase tracking-wide text-slate-600">Ver balanço completo <DisclosureChevron/></summary><div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-2 text-xs"><div><p className="text-slate-500">Você percorreu <strong className="text-slate-800">{formatKm(visibleKm)} km</strong></p></div><div><p className="text-slate-500">Seu carro faz <strong>{dailyConsumption>0?`${formatKm(dailyConsumption)} km/L`:"—"}</strong></p></div><div><p className="text-slate-500"><strong>{formatCostPerKm(dailyCostPerKm)}</strong> pagos por KM</p></div><div><p className="text-slate-500"><strong>{formatCostPerKm(dailyGrossPerKm)}</strong> ganhos por KM</p></div><div><p className="text-slate-500"><strong>{formatCostPerKm(dailyProfitPerKm)}</strong> de lucro por KM</p></div><div><p className="text-slate-500">Lucro líquido <strong className={dailyProfit<0?"text-red-700":"text-emerald-700"}>{formatBRL(dailyProfit)}</strong></p></div></div></details>
  </section>
 </div>
