...
export default function EntryForm({initialDate=todayISO(),initialMonthProfit=0}:Props){
 ...
 function saveDraft(){
  const draft:DraftState={date,mode,netFare,netApp,revenueItems,gas,alcohol,gasPrice,alcoholPrice,kmInitial,kmFinal,fuelConsumption,hoursSegments,maintenanceItems,extras};
 ...
 const fuelPrice=fuelConsumption>0?(gas>0&&gasPrice>0?gasPrice:alcohol>0&&alcoholPrice>0?alcoholPrice:weightedFuelPrice):0;
 const fuelConsumedLiters=kmDriven>0&&fuelConsumption>0?kmDriven/fuelConsumption:0;
 ...
 async function handleSubmit(e:FormEvent){
  ...
  setStatus("Salvando...");
  const payload={date,gross_amount:mode==="withFee"?revenueSummary.bruto:null,fee_percent:mode==="withFee"?revenueSummary.taxaPercentual:null,net_fare:mode==="net"?netFare:revenueSummary.liquido,revenue_details:mode==="withFee"?revenueItems:[{...createRevenueItem(),app:netApp,bruto:netFare,taxa:0}],gas_expense:gas,alcohol_expense:alcohol,gasoline_price_per_liter:gasPrice,alcohol_price_per_liter:alcoholPrice,gasoline_liters:gasLiters,alcohol_liters:alcoholLiters,km_initial:kmInitial,km_final:kmFinal,km_driven:kmDriven,hours_worked:hours,fuel_consumption_km_per_liter:fuelConsumption,fuel_consumed_liters:fuelConsumedLiters,fuel_consumed_cost:fuelConsumedCost,fuel_remaining_liters:fuelRemainingLiters,fuel_remaining_value:fuelRemainingValue,maintenance_expense:maintenanceTotal,maintenance_details:maintenanceItems.filter(m=>m.description.trim()!==""),extra_expenses:extras.filter(e=>e.name.trim()!=="")};
