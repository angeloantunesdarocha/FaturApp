...
export default function EntryForm({initialDate=todayISO(),initialMonthProfit=0}:Props){
 ...
 async function handleSubmit(e:FormEvent){
  ...
  if(totalFuelPurchase>0&&kmDriven>0&&fuelConsumption<=0){setStatus("❌ Informe o consumo médio do veículo (Km/L).");return;}
  ...
  setStatus("Salvando...");
  const payload={date,gross_amount:mode==="withFee"?revenueSummary.bruto:null,fee_percent:mode==="withFee"?revenueSummary.taxaPercentual:null,net_fare:mode==="net"?netFare:revenueSummary.liquido,revenue_details:mode==="withFee"?revenueItems:[{...createRevenueItem(),app:netApp,nomeAppPersonalizado:netApp==="Outro"?netCustomApp:"",bruto:netFare,taxa:0}],gas_expense:gas,alcohol_expense:alcohol,gasoline_price_per_liter:gasPrice,alcohol_price_per_liter:alcoholPrice,gasoline_liters:gasLiters,alcohol_liters:alcoholLiters,km_initial:kmInitial,km_final:kmFinal,km_driven:kmDriven,hours_worked:hours,fuel_consumption_km_per_liter:fuelConsumption,fuel_consumed_liters:fuelConsumedLiters,fuel_consumed_cost:fuelConsumedCost,fuel_remaining_liters:fuelRemainingLiters,fuel_remaining_value:fuelRemainingValue,maintenance_expense:maintenanceTotal,maintenance_details:maintenanceItems.filter(m=>m.description.trim()!==""),extra_expenses:extras.filter(e=>e.name.trim()!=="")};
