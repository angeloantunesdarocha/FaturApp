...
export default function EntryForm({initialDate=todayISO(),initialMonthProfit=0}:Props){
 ...
 return <div className="flex flex-col gap-4"><form onSubmit={handleSubmit} className="order-2 space-y-3">
  ...
  <details className="group rounded-xl border border-slate-200 bg-white shadow-sm"><summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3"><span className="text-sm font-bold text-slate-800">💰 Receitas <span className="ml-1 text-xs font-normal text-slate-500">{revenueItems.filter(item=>item.bruto>0).length+(netFare>0?1:0)+netRevenueItems.filter(item=>item.bruto>0).length} lanç.</span></span><strong className="text-sm text-emerald-700">{formatBRL(fareNet)} ‹</strong></summary><div className="space-y-3 border-t border-slate-100 p-3">
   ...
   {mode==="withFee"&&<> <div className="space-y-2">{revenueItems.map((item,index)=><details key={item.id} open={!item.bruto?true:undefined} className="rounded-lg border border-slate-200 bg-slate-50/70"><summary className="flex cursor-pointer list-none items-center justify-between gap-2 px-3 py-2 text-sm"><span className="truncate font-medium text-slate-700">{item.app==="Outro"?(item.nomeAppPersonalizado||"Outro aplicativo"):item.app}</span><strong className="text-emerald-700">{formatBRL(summarizeRevenue([item]).liquido)}</strong></summary><div className="space-y-3 border-t border-slate-200 p-3">
  ...
  <details className="group rounded-xl border border-slate-200 bg-white shadow-sm"><summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3"><span className="text-sm font-bold text-slate-800">⏱️ Horas trabalhadas <span className="ml-1 text-xs font-normal text-slate-500">{hoursSegments.filter(segment=>segment.start||segment.end).length} períodos</span></span><strong className="text-sm text-slate-700">{hours.toLocaleString("pt-BR",{maximumFractionDigits:2})} h ‹</strong></summary><div className="space-y-2 border-t border-slate-100 p-3">{hoursSegments.map((segment,index)=><details key={index} open={!segment.start||!segment.end?true:undefined} className="rounded-lg border border-slate-200"><summary className="flex cursor-pointer list-none items-center justify-between px-3 py-2 text-sm"><span className="font-medium text-slate-700">{segment.start||"Início"} → {segment.end||"Fim"}</span>{hoursSegments.length>1&&<button type="button" onClick={event=>{event.preventDefault();removeSegment(index);}} className="text-xs font-bold text-rose-600">✕</button>}</summary><div className="grid grid-cols-[1fr_auto_1fr] items-end gap-2 border-t border-slate-100 p-2"><div><label className="text-xs text-slate-500">Início</label><input type="time" className="input" value={segment.start} onChange={e=>updateSegment(index,{start:e.target.value})}/></div><span className="pb-3 text-slate-500">até</span><div><label className="text-xs text-slate-500">Fim</label><input type="time" className="input" value={segment.end} onChange={e=>updateSegment(index,{end:e.target.value})}/></div></div></details>)}<button type="button" onClick={addSegment} className="btn btn-secondary w-full">+ Adicionar outro período</button></div></details>
  <details className="group rounded-xl border border-slate-200 bg-white shadow-sm"><summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3"><span className="text-sm font-bold text-slate-800">🚗 Quilometragem</span><strong className="text-sm text-slate-700">{formatKm(kmDriven)} km ‹</strong></summary><div className="space-y-3 border-t border-slate-100 p-3"><div className="grid grid-cols-2 gap-3"><div><label className="label">Km inicial</label><input type="number" step="0.1" min="0" className="input" value={kmInitial||""} onChange={e=>setKmInitial(toNumber(e.target.value))} placeholder="Ex.: 52.340"/></div><div><label className="label">Km final</label><input type="number" step="0.1" min="0" className="input" value={kmFinal||""} onChange={e=>setKmFinal(toNumber(e.target.value))} placeholder="Ex.: 52.520"/></div></div>
   <div><label className="label">Consumo médio (km/L)</label><input type="number" inputMode="decimal" step="0.1" min="0" className="input" value={fuelConsumption||""} onChange={e=>setFuelConsumption(toNumber(e.target.value))} placeholder="Automático pelos abastecimentos"/><p className="mt-1 text-xs text-slate-500">Informe ou deixe o FaturApp calcular.</p></div></div></details>
  <details className="group rounded-xl border border-slate-200 bg-white shadow-sm"><summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3"><span className="text-sm font-bold text-slate-800">⛽ Combustível <span className="ml-1 text-xs font-normal text-slate-500">{(gas>0?1:0)+(alcohol>0?1:0)+fuelPurchases.filter(item=>item.amount>0).length} lanç.</span></span><strong className="text-sm text-slate-700">{formatBRL(totalFuelPurchase)} ‹</strong></summary><div className="space-y-2 border-t border-slate-100 p-3">
   ...
   <details className="rounded-lg border border-slate-200"><summary className="flex cursor-pointer list-none items-center justify-between px-3 py-2 text-sm"><span className="font-medium text-slate-700">Álcool{alcoholPrice>0?` · ${formatBRL(alcoholPrice)}/L`:""}</span><strong>{formatBRL(alcohol)}</strong></summary><div className="grid grid-cols-2 gap-2 border-t border-slate-100 p-2"><div><label className="label">Preço/L (R$)</label><input type="number" step="0.001" min="0" className="input" value={alcoholPrice||""} onChange={e=>setAlcoholPrice(toNumber(e.target.value))} placeholder="4,39"/></div><div><label className="label">Abastecido (R$)</label><input type="number" step="0.01" min="0" className="input" value={alcohol||""} onChange={e=>setAlcohol(toNumber(e.target.value))} placeholder="80,00"/></div></div></details>
    {fuelPurchases.map((purchase,index)=><details key={purchase.id} open={!purchase.amount||!purchase.pricePerLiter?true:undefined} className="rounded-lg border border-slate-200 bg-slate-50/70"><summary className="flex cursor-pointer list-none items-center justify-between px-3 py-2 text-sm"><span className="truncate font-medium text-slate-700">{purchase.type==="gasoline"?"Gasolina":"Álcool"}{purchase.pricePerLiter>0?` · ${formatBRL(purchase.pricePerLiter)}/L`:""}</span><strong>{formatBRL(purchase.amount)}</strong></summary><div className="space-y-3 border-t border-slate-200 p-3">
 ...
 <div className="sticky top-2 z-10 order-1">
  <section className={"rounded-xl border p-3 shadow-md backdrop-blur-sm " + (summaryTone==="profit"?"border-emerald-200 bg-emerald-50/95":summaryTone==="loss"?"border-red-200 bg-red-50/95":"border-amber-200 bg-amber-50/95")} aria-live="polite" aria-label="Resumo financeiro diário">
   ...
   <div className="mt-2 grid grid-cols-3 divide-x divide-slate-200/80 border-t border-slate-200/80 pt-2">
    ...
    <div className="pl-2"><p className="text-base font-black text-slate-800">{effectiveFuelConsumption>0?formatKm(effectiveFuelConsumption):"—"}<span className="ml-0.5 text-[10px] font-bold text-slate-500">km/L</span></p><p className="text-[9px] font-bold uppercase tracking-wide text-slate-500">CONSUMO</p></div>
   </div>
   ...
   <details className="mt-2 border-t border-slate-200/80 pt-2"><summary className="cursor-pointer list-none text-center text-[10px] font-bold uppercase tracking-wide text-slate-600">Ver balanço completo ▾</summary><div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-2 text-xs"><div><p className="text-slate-500">Ganhos líquidos</p><strong className="text-emerald-700">{formatBRL(fareNet)}</strong></div><div><p className="text-slate-500">Distância</p><strong>{formatKm(visibleKm)} km</strong></div><div><p className="text-slate-500">Abastecido</p><strong>{formatBRL(totalFuelPurchase)} · {formatLiters(totalPurchasedLiters)} L</strong></div><div><p className="text-slate-500">Preço médio/L</p><strong>{weightedFuelPrice>0?formatBRL(weightedFuelPrice):"—"}</strong></div><div><p className="text-slate-500">Combustível consumido</p><strong>{formatBRL(fuelConsumedCost)} · {formatLiters(fuelConsumedLiters)} L</strong></div><div><p className="text-slate-500">Restante no tanque</p><strong>{formatLiters(fuelRemainingLiters)} L · {formatBRL(fuelRemainingValue)}</strong></div><div><p className="text-slate-500">Manutenção</p><strong>{formatBRL(maintenanceTotal)}</strong></div><div><p className="text-slate-500">Gastos extras</p><strong>{formatBRL(extrasSum)}</strong></div><div><p className="text-slate-500">Taxas dos apps</p><strong>{formatBRL(feeAmount)}</strong></div><div><p className="text-slate-500">Lucro por km</p><strong>{visibleProfitPerKm!==null?formatBRL(visibleProfitPerKm):"—"}</strong></div><div className="col-span-2 flex items-center justify-between border-t border-slate-200 pt-2"><span className="font-semibold text-slate-600">Total de despesas</span><strong className="text-rose-700">{formatBRL(totalExpenses)}</strong></div></div></details>
  </section>
 </div>
