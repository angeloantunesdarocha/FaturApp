...
export default function EntryForm({initialDate=hojeBrasilia(),initialMonthProfit=0,userId}:Props){
 ...
 <section className="order-4 rounded-xl border border-slate-200 bg-slate-50/80 p-3 shadow-sm" aria-label="Resumo do dia até agora">
  ...
  {hasSavedLaunches&&<div className="mt-3 grid grid-cols-2 gap-2">
   ...
   <div className="rounded-lg border border-slate-200 bg-white px-2.5 py-2"><p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">🚗 Consumo médio</p><strong className="mt-1 block text-sm font-black text-slate-800">{consolidatedSavedMetrics.consumption>0?`${formatKm(consolidatedSavedMetrics.consumption)} km/L`:"—"}</strong></div>
