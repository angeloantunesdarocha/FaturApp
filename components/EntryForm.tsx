...
export default function EntryForm({initialDate=todayISO(),initialMonthProfit=0}:Props){
 ...
 const automaticFuelConsumption=kmDriven>0&&totalPurchasedLiters>0?kmDriven/totalPurchasedLiters:0;
 const effectiveFuelConsumption=informedFuelConsumption>0?informedFuelConsumption:automaticFuelConsumption;
 const isAutomaticFuelConsumption=informedFuelConsumption<=0&&automaticFuelConsumption>0;
 ...
 return <div className="flex flex-col gap-4"><form onSubmit={handleSubmit} className="order-2 space-y-3">
  ...
  <details className="group rounded-xl border border-slate-200 bg-white shadow-sm"><summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3"><span className="text-sm font-bold text-slate-800">🚗 Quilometragem</span><span className="flex items-center"><strong className="text-sm text-slate-700">{formatKm(kmDriven)} km</strong><DisclosureChevron/></span></summary><div className="space-y-3 border-t border-slate-100 p-3"><div className="grid grid-cols-2 gap-3"><div><label className="label">Km inicial</label><input type="number" step="0.1" min="0" className="input" value={kmInitial||""} onChange={e=>setKmInitial(toNumber(e.target.value))} placeholder="Ex.: 52.340"/></div><div><label className="label">Km final</label><input type="number" step="0.1" min="0" className="input" value={kmFinal||""} onChange={e=>setKmFinal(toNumber(e.target.value))} placeholder="Ex.: 52.520"/></div></div>
   <div><label className="label">Consumo médio do veículo (km/L)</label><input type="number" inputMode="decimal" step="0.1" min="0" className="input" value={fuelConsumption||""} onChange={e=>updateVehicleConsumption(toNumber(e.target.value))} placeholder="Automático ou ex.: 10 km/L"/><p className="mt-1 text-xs text-slate-500">Informe o km/L ou deixe em branco para calcular automaticamente.</p></div></div></details>
 ...
 <div className="sticky top-2 z-10 order-1">
  <section className={"rounded-xl border p-3 shadow-md backdrop-blur-sm " + (summaryTone==="profit"?"border-emerald-200 bg-emerald-50/95":summaryTone==="loss"?"border-red-200 bg-red-50/95":"border-amber-200 bg-amber-50/95")} aria-live="polite" aria-label="Resumo financeiro diário">
   ...
   <div className="mt-2 grid grid-cols-3 divide-x divide-slate-200/80 border-t border-slate-200/80 pt-2">
    ...
    <div className="pl-2"><p className="text-base font-black text-slate-800">{effectiveFuelConsumption>0?formatKm(effectiveFuelConsumption):"—"}<span className="ml-0.5 text-[10px] font-bold text-slate-500">km/L</span></p><p className="text-[9px] font-bold uppercase tracking-wide text-slate-500">{isAutomaticFuelConsumption?"KM/L AUTOMÁTICO":"KM POR LITRO"}</p></div>
   </div>
   {(totalPurchasedLiters>0||kmDriven>0)&&<div className="mt-2 grid grid-cols-2 gap-2 border-t border-slate-200/80 pt-2">
    <div className="rounded-lg border border-emerald-200/70 bg-white/60 px-2 py-1.5"><p className="text-[9px] font-bold uppercase tracking-wide text-slate-500">{isAutomaticFuelConsumption?"Abastecido no período":`Consumido em ${formatKm(kmDriven)} km`}</p><p className="text-sm font-black text-slate-800">{effectiveFuelConsumption>0?`${formatLiters(isAutomaticFuelConsumption?totalPurchasedLiters:fuelConsumedLiters)} L`:"—"}</p><p className="text-[11px] font-semibold text-emerald-700">{effectiveFuelConsumption>0?formatBRL(isAutomaticFuelConsumption?totalFuelPurchase:fuelConsumedCost):"Aguardando dados"}</p></div>
    <div className="rounded-lg border border-sky-200/70 bg-white/60 px-2 py-1.5"><p className="text-[9px] font-bold uppercase tracking-wide text-slate-500">Restante no tanque</p><p className="text-sm font-black text-slate-800">{informedFuelConsumption>0?`${formatLiters(fuelRemainingLiters)} L`:"—"}</p><p className="text-[11px] font-semibold text-sky-700">{informedFuelConsumption>0?formatBRL(fuelRemainingValue):isAutomaticFuelConsumption?"Requer km/L informado":"Aguardando dados"}</p></div>
   </div>}
   {totalPurchasedLiters>0&&<p className="mt-1 text-center text-[10px] text-slate-500">Abastecido: {formatLiters(totalPurchasedLiters)} L · {formatBRL(totalFuelPurchase)}{weightedFuelPrice>0?` · ${formatBRL(weightedFuelPrice)}/L`:""}</p>}
   {isAutomaticFuelConsumption&&<p className="mt-1 text-center text-[10px] text-slate-500">Média estimada: quilômetros rodados ÷ litros abastecidos.</p>}
   ...
   <details className="group mt-2 border-t border-slate-200/80 pt-2"><summary className="flex cursor-pointer list-none items-center justify-center text-[10px] font-bold uppercase tracking-wide text-slate-600">Ver balanço completo <DisclosureChevron/></summary><div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-2 text-xs"><div><p className="text-slate-500">Ganhos líquidos</p><strong className="text-emerald-700">{formatBRL(fareNet)}</strong></div><div><p className="text-slate-500">Distância</p><strong>{formatKm(visibleKm)} km</strong></div><div><p className="text-slate-500">Abastecido</p><strong>{formatBRL(totalFuelPurchase)} · {formatLiters(totalPurchasedLiters)} L</strong></div><div><p className="text-slate-500">Preço médio/L</p><strong>{weightedFuelPrice>0?formatBRL(weightedFuelPrice):"—"}</strong></div><div><p className="text-slate-500">{isAutomaticFuelConsumption?"Combustível abastecido":"Combustível consumido"}</p><strong>{effectiveFuelConsumption>0?`${formatBRL(isAutomaticFuelConsumption?totalFuelPurchase:fuelConsumedCost)} · ${formatLiters(isAutomaticFuelConsumption?totalPurchasedLiters:fuelConsumedLiters)} L`:"Aguardando dados"}</strong></div><div><p className="text-slate-500">Restante no tanque</p><strong>{informedFuelConsumption>0?`${formatLiters(fuelRemainingLiters)} L · ${formatBRL(fuelRemainingValue)}`:"Requer km/L informado"}</strong></div><div><p className="text-slate-500">Manutenção</p><strong>{formatBRL(maintenanceTotal)}</strong></div><div><p className="text-slate-500">Gastos extras</p><strong>{formatBRL(extrasSum)}</strong></div><div><p className="text-slate-500">Taxas dos apps</p><strong>{formatBRL(feeAmount)}</strong></div><div><p className="text-slate-500">Lucro por km</p><strong>{visibleProfitPerKm!==null?formatBRL(visibleProfitPerKm):"—"}</strong></div><div className="col-span-2 flex items-center justify-between border-t border-slate-200 pt-2"><span className="font-semibold text-slate-600">Total de despesas</span><strong className="text-rose-700">{formatBRL(totalExpenses)}</strong></div></div></details>
  </section>
 </div>
