...
export default function EntryForm({initialDate=todayISO(),initialMonthProfit=0}:Props){
 ...
 const consolidatedSavedMetrics=useMemo(()=>draftMetrics(mergeDrafts(date,savedDrafts)),[date,savedDrafts]);
 const consolidatedSavedProfit=consolidatedSavedMetrics.revenue.liquido-(consolidatedSavedMetrics.consumed>0?consolidatedSavedMetrics.consumedCost:consolidatedSavedMetrics.gas+consolidatedSavedMetrics.alcohol)-consolidatedSavedMetrics.maintenanceTotal-consolidatedSavedMetrics.extrasTotal;
 ...
 return <div className="flex flex-col gap-4"><form onSubmit={handleSubmit} className="order-2 space-y-3">
  ...
  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2"><button type="button" onClick={saveDraft} className="btn btn-secondary w-full">Salvar Lançamento</button><button type="submit" className="btn btn-primary w-full">Registrar e fechar dia</button></div>{status&&<p className={"text-sm text-center " + (editingLaunchId?"font-bold text-amber-700":"text-slate-600")}>{status}</p>}
 </fieldset></form>
