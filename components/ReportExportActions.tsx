...
type Row = {
  ...
  maintenance: { description: string; value: number }[];
...
function details(items: any[] | undefined) {
...
function makePdf(rows: Row[], from: string, to: string) {
  const totals = rows.reduce((a, r) => ({
    ...
    fee: a.fee + r.feeAmount,
    net: a.net + r.net,
    costs: a.costs + r.costs,
    profit: a.profit + r.profit,
    km: a.km + r.km,
    hours: a.hours + r.hours,
  }), { gross: 0, fee: 0, net: 0, costs: 0, profit: 0, km: 0, hours: 0 });
  ...
  autoTable(doc, {
    head: [["Data", "Horas", "Km", "Receita bruta", "Taxa app", "Valor taxa", "Receita líquida", "Custos", "Lucro líquido", "Lucro/km"]],
    body: rows.map(r => [formatDateBR(r.entry.date), `${nfmt(r.hours)} h`, `${nfmt(r.km, 0)} km`, formatBRL(r.gross), `${r.feePercent.toFixed(2)}%`, formatBRL(r.feeAmount), formatBRL(r.net), formatBRL(r.costs), formatBRL(r.profit), money(r.profitKm)]),
    startY: 33,
    theme: "striped",
  ...
  const addDetails = (title: string, list: { date: string; description: string; value: number }[]) => {
    ...
    doc.setFontSize(11); doc.setFont(undefined, "bold"); doc.text(title, 14, y); y += 3;
    autoTable(doc, {
...
export default function ReportExportActions({ entries, from, to }: Props) {
  ...
  const rows = useMemo<Row[]>(() => entries.filter(e => e.date >= from && e.date <= to).map(entry => {
    ...
    const maintenance = details(entry.maintenance_details);
    const extras = details(entry.extra_expenses);
    const gas = Math.max(0, Number(entry.gas_expense) || 0);
    const alcohol = Math.max(0, Number(entry.alcohol_expense) || 0);
    const maintenanceValue = maintenance.reduce((s, x) => s + x.value, 0) || Math.max(0, Number(entry.maintenance_expense) || 0);
    const extrasValue = extras.reduce((s, x) => s + x.value, 0);
    const costs = gas + alcohol + maintenanceValue + extrasValue;
  ...
  const text = useMemo(() => {
    const lines = ["*FaturApp — Relatório completo*", `Período: ${formatDateBR(from)} até ${formatDateBR(to)}`, ""];
    rows.forEach(r => {
      ...
      lines.push(`Taxa do app: ${r.feePercent.toFixed(2)}% | Valor da taxa: ${formatBRL(r.feeAmount)}`);
      lines.push(`Receita líquida: ${formatBRL(r.net)} | Custos: ${formatBRL(r.costs)}`);
      lines.push(`Lucro líquido: ${formatBRL(r.profit)} | Lucro/km: ${money(r.profitKm)}`);
  ...
  async function emailPdf() {
    ...
    try {
      ...
      doc.save(file.name);
      window.location.href = `mailto:?subject=${encodeURIComponent("Relatório FaturApp")}&body=${encodeURIComponent("O relatório em PDF foi baixado. Anexe o arquivo PDF a este e-mail.")}`;
  ...
  return <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-emerald-600">Exportar relatório</p><h2 className="mt-1 text-xl font-extrabold text-slate-900">Relatório completo do período</h2><p className="mt-1 text-sm text-slate-500">PDF, tabela detalhada para WhatsApp ou PDF para envio por e-mail.</p></div>
