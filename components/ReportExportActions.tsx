...
type RevenueDetail = { app: string; gross: number; feePercent: number; feeAmount: number; net: number };
type Row = {
  entry: DailyEntry;
  gross: number;
  feePercent: number;
...
type Totals = { gross: number; fee: number; net: number; costs: number; profit: number; km: number; hours: number; maintenance: number; extras: number; fuel: number };
...
/** Normaliza as receitas sem usar || para valores monetários válidos como R$ 0,00. */
function revenueDetails(entry: DailyEntry): RevenueDetail[] {
  const items = entry.revenue_details || [];
  if (items.length) {
    return items.map((item) => {
      const gross = Math.max(0, Number(item.bruto) || 0);
      const feePercent = Math.max(0, Number(item.taxa) || 0);
      const rawFee = Number(item.taxaValor);
      const feeAmount = Number.isFinite(rawFee) ? Math.max(0, rawFee) : Math.max(0, Math.round(gross * feePercent) / 100);
      const rawNet = Number(item.liquido);
      const net = Number.isFinite(rawNet) ? Math.max(0, rawNet) : Math.max(0, Math.round((gross - feeAmount) * 100) / 100);
      return {
  ...
  const totals = computeRevenueTotals(entry);
  if (totals.gross <= 0 && totals.net <= 0) return [];
  const feePercent = totals.gross > 0 ? (totals.fee / totals.gross) * 100 : 0;
  return [{ app: "Indefinido", gross: totals.gross, feePercent, feeAmount: totals.fee, net: totals.net }];
}

function calcTotals(rows: Row[]): Totals {
  return rows.reduce<Totals>((acc, r) => ({
    gross: acc.gross + safe(r.gross), fee: acc.fee + safe(r.feeAmount), net: acc.net + safe(r.net), costs: acc.costs + safe(r.costs), profit: acc.profit + safe(r.profit),
    km: acc.km + safe(r.km), hours: acc.hours + safe(r.hours), maintenance: acc.maintenance + safe(r.maintenanceTotal), extras: acc.extras + safe(r.extrasTotal), fuel: acc.fuel + safe(r.fuelTotal),
...
function makePdf(rows: Row[], from: string, to: string): jsPDF {
  ...
  doc.setFontSize(18); doc.setFont("helvetica", "bold"); doc.text("FaturApp — Relatório Completo", 14, 14);
  doc.setFontSize(9); doc.setFont("helvetica", "normal"); doc.text(`Período: ${formatDateBR(from)} até ${formatDateBR(to)}   ·   ${rows.length} dia(s) analisado(s)`, 14, 21);

  autoTable(doc, {
    head: [["Data", "Horas", "Km", "Receita Bruta Total", "Receita Líquida Total", "Custos Totais", "Lucro Líquido", "Lucro/km"]],
    body: rows.map((r) => [formatDateBR(r.entry.date), `${nfmt(r.hours)} h`, `${nfmt(r.km, 0)} km`, formatBRL(r.gross), formatBRL(r.net), formatBRL(r.costs), formatBRL(r.profit), money(r.profitKm)]),
    foot: [[
      { content: "TOTAL", styles: { halign: "left" } }, { content: `${nfmt(t.hours)} h`, styles: { halign: "right" } }, { content: `${nfmt(t.km, 0)} km`, styles: { halign: "right" } },
      { content: formatBRL(t.gross), styles: { halign: "right" } }, { content: formatBRL(t.net), styles: { halign: "right" } }, { content: formatBRL(t.costs), styles: { halign: "right" } },
      { content: formatBRL(t.profit), styles: { halign: "right" } }, { content: money(profitKmAvg), styles: { halign: "right" } },
    ]],
    startY: 27, theme: "striped", showFoot: "lastPage", styles: { fontSize: 7, cellPadding: 2, overflow: "linebreak" },
    ...
    columnStyles: { 0: { cellWidth: 25, halign: "left" }, 1: { cellWidth: 20, halign: "right" }, 2: { cellWidth: 20, halign: "right" }, 3: { cellWidth: 32, halign: "right" }, 4: { cellWidth: 34, halign: "right" }, 5: { cellWidth: 30, halign: "right" }, 6: { cellWidth: 32, halign: "right" }, 7: { cellWidth: 27, halign: "right" } },
  });

  let y = (doc as any).lastAutoTable.finalY + 8;
  const ensureSpace = (height = 35) => { if (y > 190 - height) { doc.addPage(); y = 15; } };
  const addSectionTitle = (title: string) => { ensureSpace(25); doc.setFontSize(11); doc.setFont("helvetica", "bold"); doc.text(title, 14, y); y += 4; };
  ...
  if (revenueRows.length) {
    ...
    autoTable(doc, {
      ...
      foot: [[{ content: "TOTAL", styles: { halign: "left" } }, { content: "" }, { content: formatBRL(t.gross), styles: { halign: "right" } }, { content: "" }, { content: formatBRL(t.fee), styles: { halign: "right" } }, { content: formatBRL(t.net), styles: { halign: "right" } }]],
      startY: y, theme: "grid", showFoot: "lastPage", styles: { fontSize: 7.5, cellPadding: 2 }, headStyles: { fillColor: [15, 45, 74], textColor: [255, 255, 255], halign: "center" }, footStyles: { fillColor: [22, 101, 52], textColor: [255, 255, 255], fontStyle: "bold" },
      columnStyles: { 0: { cellWidth: 25, halign: "left" }, 1: { halign: "left" }, 2: { cellWidth: 32, halign: "right" }, 3: { cellWidth: 22, halign: "center" }, 4: { cellWidth: 38, halign: "right" }, 5: { cellWidth: 34, halign: "right" } },
    });
  ...
  const fuelRows = rows.flatMap((r) => {
    ...
    const gasLiters = Math.max(0, Number(r.entry.gasoline_liters) || 0), alcoholLiters = Math.max(0, Number(r.entry.alcohol_liters) || 0);
    const gasValue = Math.max(0, Number(r.entry.gas_expense) || 0), alcoholValue = Math.max(0, Number(r.entry.alcohol_expense) || 0);
  ...
  if (fuelRows.length) {
    ...
    autoTable(doc, {
      ...
      foot: [[{ content: "TOTAL", styles: { halign: "left" } }, { content: "" }, { content: `${nfmt(fuelRows.reduce((s, x) => s + x.liters, 0), 3)} L`, styles: { halign: "right" } }, { content: formatBRL(t.fuel), styles: { halign: "right" } }]],
      startY: y, theme: "grid", showFoot: "lastPage", styles: { fontSize: 8, cellPadding: 2 }, headStyles: { fillColor: [15, 45, 74], textColor: [255, 255, 255], halign: "center" }, footStyles: { fillColor: [22, 101, 52], textColor: [255, 255, 255], fontStyle: "bold" },
      columnStyles: { 0: { cellWidth: 30, halign: "left" }, 1: { halign: "left" }, 2: { cellWidth: 45, halign: "right" }, 3: { cellWidth: 45, halign: "right" } },
    });
    y = (doc as any).lastAutoTable.finalY + 7;
  }
  ...
  const addDetailSection = (title: string, list: { date: string; description: string; value: number }[]) => {
    ...
    autoTable(doc, { head: [["Data", "Descrição", "Valor"]], body: list.map((x) => [formatDateBR(x.date), x.description, formatBRL(x.value)]), foot: [[{ content: "", styles: { halign: "left" } }, { content: "TOTAL", styles: { halign: "right", fontStyle: "bold" } }, { content: formatBRL(sectionTotal), styles: { halign: "right", fontStyle: "bold" } }]], startY: y, theme: "grid", showFoot: "lastPage", styles: { fontSize: 8, cellPadding: 2 }, headStyles: { fillColor: [15, 45, 74], textColor: [255, 255, 255], halign: "center" }, footStyles: { fillColor: [22, 101, 52], textColor: [255, 255, 255] }, columnStyles: { 0: { cellWidth: 28, halign: "left" }, 1: { halign: "left" }, 2: { cellWidth: 35, halign: "right" } } });
    y = (doc as any).lastAutoTable.finalY + 7;
  };
...
function makeText(rows: Row[], from: string, to: string): string {
  ...
  lines.push(`⏱ Total de horas: *${nfmt(t.hours)} h*`, `🛣 Total de km: *${nfmt(t.km, 0)} km*`, `💰 Receita bruta total: *${formatBRL(t.gross)}*`, `💸 Total de taxas (app): *${formatBRL(t.fee)}*`, `✅ Receita líquida total: *${formatBRL(t.net)}*`, `⛽ Total de combustível: *${formatBRL(t.fuel)}*`, `🔩 Total de manutenção: *${formatBRL(t.maintenance)}*`, `📦 Total de gastos extras: *${formatBRL(t.extras)}*`, `🔴 Total de custos: *${formatBRL(t.costs)}*`, `🟢 *Lucro líquido total: ${formatBRL(t.profit)}*`);
  if (profitKmAvg !== null) lines.push(`📍 Lucro médio por km: *${money(profitKmAvg)}*`);
  const allMaintenance = rows.flatMap((r) => r.maintenance.map((x) => ({ ...x, date: r.entry.date })));
  if (allMaintenance.length) { lines.push("", sep, "🔩 *MANUTENÇÃO — DETALHAMENTO*", sep); allMaintenance.forEach((x) => lines.push(`${formatDateBR(x.date)} | ${x.description} | ${formatBRL(x.value)}`)); }
  const allExtras = rows.flatMap((r) => r.extras.map((x) => ({ ...x, date: r.entry.date })));
  if (allExtras.length) { lines.push("", sep, "📦 *GASTOS EXTRAS — DETALHAMENTO*", sep); allExtras.forEach((x) => lines.push(`${formatDateBR(x.date)} | ${x.description} | ${formatBRL(x.value)}`)); }
...
export default function ReportExportActions({ entries, from, to }: Props) {
  ...
  const rows = useMemo<Row[]>(() => entries.filter((e) => e.date >= from && e.date <= to).sort((a, b) => a.date.localeCompare(b.date)).map((entry) => {
    ...
    const revenueTotals = computeRevenueTotals(entry);
    const gross = revenue.length ? revenue.reduce((s, x) => s + x.gross, 0) : revenueTotals.gross;
    const feeAmount = revenue.length ? revenue.reduce((s, x) => s + x.feeAmount, 0) : revenueTotals.fee;
    const feePercent = gross > 0 ? (feeAmount / gross) * 100 : 0;
    const net = revenue.length ? revenue.reduce((s, x) => s + x.net, 0) : revenueTotals.net;
    const maint = details(entry.maintenance_details);
    const extr = details(entry.extra_expenses);
    const gas = Math.max(0, Number(entry.gas_expense) || 0), alcohol = Math.max(0, Number(entry.alcohol_expense) || 0);
    ...
    const km = Math.max(0, Number(entry.km_driven) || 0), hours = Math.max(0, Number(entry.hours_worked) || 0);
