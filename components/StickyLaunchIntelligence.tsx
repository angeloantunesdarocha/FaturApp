...
type Props = {
  profit: number; costs: number; revenue: number; gross?: number; km?: number; hours?: number;
  feeAmount?: number; profitPerKm?: number; profitPerHour?: number; fuelSpent?: number;
  maintenance?: number; extras?: number; fuelRemainingValue?: number; className?: string; active?: boolean;
};
...
export default function StickyLaunchIntelligence({ profit, costs: _costs, revenue, gross = revenue, km = 0, hours = 0, feeAmount = 0, profitPerKm, profitPerHour, fuelSpent = 0, maintenance = 0, extras = 0, fuelRemainingValue = 0, className = "", active = true }: Props) {
    ...
    <div className={`fixed inset-x-0 top-0 z-[60] w-full px-1.5 pt-[env(safe-area-inset-top)] ${className}`} style={{ isolation: "isolate" }}>
      <div className={`mx-auto w-full max-w-3xl overflow-hidden rounded-b-xl border shadow-md shadow-slate-900/10 backdrop-blur-xl ${styles.shell}`}>
        <button type="button" onClick={() => setExpanded(value => !value)} aria-expanded={expanded} className="flex min-h-[50px] w-full items-center gap-1.5 px-2 py-1.5 text-left sm:min-h-[54px] sm:gap-3 sm:px-4">
          ...
          <div className="min-w-0 flex-1"><div className={`truncate text-[8px] font-bold uppercase tracking-[0.06em] ${styles.accent}`}>{styles.label}</div><div className="mt-0.5 flex items-baseline gap-1"><strong className={`text-base font-black leading-none sm:text-lg ${styles.accent}`}>{brl(profit)}</strong><span className="text-[9px] font-medium text-slate-500">lucro</span></div></div>
        ...
        {expanded && <div className={`border-t border-slate-200/60 px-1.5 pb-2 pt-1.5 ${styles.soft}`}>
          <div className="grid grid-cols-2 gap-1.5">
            <div className="space-y-1 border-r border-slate-300/40 pr-1.5">
              <Metric label="Valor bruto" value={brl(gross)} /><Metric label="Taxa do app" value={brl(feeAmount)} />
              <Metric label="Km rodados" value={`${decimal(km)} km`} /><Metric label="Valor por KM" value={brl(kmValue)} />
              <Metric label="Horas trabalhadas" value={`${decimal(hours)} h`} />
            </div>
            <div className="space-y-1 pl-0">
              <Metric label="Valor por hora" value={brl(hourValue)} /><Metric label="Combustível" value={brl(fuelSpent)} />
              <Metric label="Manutenção" value={brl(maintenance)} /><Metric label="Gastos extras" value={brl(extras)} />
...
function Metric({ label, value }: { label: string; value: string }) {
  return <div className="min-w-0 rounded-md border border-white/80 bg-white/75 px-1.5 py-1">
