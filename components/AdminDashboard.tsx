...
type User = {
  ...
  last_access_at: string | null; last_entry_at: string | null; entries_count: number;
  last_report_at: string | null; reports_count: number; contributions_count: number;
  contributions_amount: number; last_contribution_at: string | null;
};
...
export default function AdminDashboard({ data }: Props) {
  ...
  return <div className="space-y-5 pb-12">
    ...
    <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="overflow-x-auto"><table className="min-w-[1450px] w-full text-sm"><thead className="bg-slate-50 text-left text-[11px] uppercase tracking-wide text-slate-500"><tr><th className="px-4 py-3">Usuário</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Último acesso</th><th className="px-4 py-3 text-right">Lançamentos</th><th className="px-4 py-3">Último lançamento</th><th className="px-4 py-3 text-right">Relatórios</th><th className="px-4 py-3">Último relatório</th><th className="px-4 py-3">Apoios</th><th className="px-4 py-3">Último apoio</th><th className="px-4 py-3 text-right">Ação</th></tr></thead>
      <tbody>{users.map((user) => <tr key={user.id} className="border-t border-slate-100 align-top"><td className="px-4 py-4"><p className="font-bold text-slate-800">{user.login}</p><p className="text-xs text-slate-500">{user.email || "Sem e-mail"}</p></td><td className="px-4 py-4"><span className={"rounded-full px-2.5 py-1 text-xs font-bold " + (user.online ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-600")}>{user.online ? "Online agora" : "Offline"}</span><p className="mt-2 text-xs text-slate-500">{user.role}</p></td><td className="px-4 py-4 text-xs text-slate-600"><p>{dt(user.last_access_at)}</p><p className="mt-1 text-slate-400">{user.last_path || "—"}</p></td><td className="px-4 py-4 text-right font-bold">{number.format(user.entries_count)}</td><td className="px-4 py-4 text-xs text-slate-600">{dt(user.last_entry_at)}</td><td className="px-4 py-4 text-right font-bold">{number.format(user.reports_count)}</td><td className="px-4 py-4 text-xs text-slate-600">{dt(user.last_report_at)}</td><td className="px-4 py-4"><p className="font-bold">{money.format(Number(user.contributions_amount || 0))}</p><p className="text-xs text-slate-500">{number.format(user.contributions_count)} apoio(s)</p></td><td className="px-4 py-4 text-xs text-slate-600">{dt(user.last_contribution_at)}</td><td className="px-4 py-4 text-right">{user.role === "admin" ? <span className="text-xs font-bold text-slate-400">Protegido</span> : <button disabled={pending} onClick={() => removeUser(user)} className="rounded-lg border border-rose-200 px-3 py-2 text-xs font-bold text-rose-700 hover:bg-rose-50 disabled:opacity-50">Excluir</button>}</td></tr>)}</tbody></table></div>
    </section>
